...
async function pushHaSensors(checkResults, trigger = 'manual') {
  ...
  await pushState('sensor.proxmoxadminpanel_panel_version', CURRENT_VERSION,
    { friendly_name: 'Proxmox Admin Panel — Version', icon: 'mdi:tag' });
  await pushState('sensor.proxmoxadminpanel_update_available', latestVersion && latestVersion !== CURRENT_VERSION ? 'yes' : 'no',
    { friendly_name: 'Proxmox Admin Panel — Update Available', icon: 'mdi:update',
